...
Scaffolds a complete block directory — the standard collaboration unit in a block-structured project. For the full block definition, refer to `resources/BLOCK_DEFINITION.md` bundled in this plugin (sibling of this `skills/` folder).
...
> Copy `resources/BLOCK_INTAKE.md` from this plugin into your project, fill it in, and paste it back. The agent will scaffold everything from it.
...
runtime_info:
  input:
    <input_name>: human  # <description>; `human` = user must fill, "" = env/auto-supplied
    # one entry per declared input; omit section if none

  output:
    <output_name>:       # one entry per declared output; omit section if none
      description: <description>
      path: <block-relative path>   # static outputs; use `value: null` for run-produced ones
```
...
```bash
cp <plugin_root>/resources/archive_run.sh <new_block>/scripts/archive_run.sh
