...
| 5 | **Cloudflare deploy credentials — root docs only** (root scope only; the root `scripts/dryrun.sh` runs this): `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` available in the environment or in `~/.config/trajgen_progress_cloudflare.env`, for `docs/deploy_cloudflare_pages.sh`. Missing → warning. This check does **not** cover per-block dashboard publishing — see the next row. | `env:cloudflare-config` (warning) |
| 5b | **Cloudflare deploy credentials — per-block dashboards** (optional, surfaced automatically): curator/tracer/evaluator each check their own dashboard-publishing prerequisites (an `npx`/node toolchain plus that block's own credential env file — they are **not** the same file as row 5, each block uses a different name) inside their own `scripts/dryrun.sh`. Tracer and evaluator publish via a long-running `dashboard/run_cloudflare_pages_sync.sh` loop; curator publishes via a manual `wrangler pages deploy` from `dashboard/site/` (see its `dashboard/README.md`). Trainer instead checks for a `cloudflared` binary (its dashboard uses a Cloudflare quick tunnel, no account credentials). Step 3 below already runs every block's `dryrun.sh` and folds every `OK`/`WARN` line into the report — no separate root-level probe is needed for these. Always a warning, **never required** — local HTML dashboards work regardless. Suggest `/root:setup`'s optional Cloudflare extra as the fix. | `dryrun:warn` (warning, via each block's own dryrun.sh) |
