...
All commands run from the block root `blocks/tracer/`.
...
- `repos/harbor` is registered as a tracked git submodule (`.gitmodules`
  in the repo root); a fresh clone of LegoFlow therefore needs
  `git submodule update --init blocks/tracer/repos/harbor` before
