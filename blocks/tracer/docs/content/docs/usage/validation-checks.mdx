...
Use this page when `/tracer:check` reports a problem and you want to understand
what Tracer is validating. The slash command is the user-facing preflight: it
checks task sources, repo pins, runtime environments, model endpoint, LiteLLM
proxy port, Docker, agent runtime, and conversion settings before rollouts
start.

The shell tests below are the lower-level version of that contract. Most users
do not need to run them directly; they are useful when changing Tracer internals,
plugin skills, dashboard behavior, or CI.
...
| `02_repo_pins.sh` | `repos/harbor` and `repos/LegoFlow-Trace-Crafter`. | Both repos exist, match configured pins, keep expected origins, and have clean worktrees. |
| `03_uv_envs_editable.sh` | Harbor, LiteLLM, and `legoflow_trace_crafter` environments. | Expected envs exist; editable/import checks pass. `legoflow_trace_crafter` may skip when SFT conversion is disabled. |
