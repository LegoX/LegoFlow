...
The generator is `dashboard/progress_monitor.py`. It carries [PEP 723](https://peps.python.org/pep-0723/) inline metadata and a `uv run` shebang, so it needs no `pip install` — just `uv` on `PATH`. Run from `blocks/tracer/`:
