...
Use the shared commands `/tracer:check`, `/tracer:setup`, `/tracer:run`, and `/tracer:dashboard`. Claude Code and Codex use the same command names; the executable fallback is `./bin/legoflow /tracer:check`.
