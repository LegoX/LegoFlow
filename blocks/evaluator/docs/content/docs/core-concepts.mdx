...
- **Mode B — local checkpoint**: serve a trained checkpoint with vLLM on a GPU node (`scripts/serve_local_model.sh`) and point `llm_api` at it. See [Local Model](/docs/local-model).
