...
Use this page when `/evaluator:check` reports a problem and you want to
understand what Evaluator is validating. The slash command is the user-facing
preflight: it checks the benchmark registry, Harbor checkout, Python
environments, LiteLLM setup, runtime images, agent runtime, model endpoint, and
launch gates before an evaluation spends model tokens.

The shell tests below are the lower-level version of that contract. Most users
do not need to run them directly; they are useful when changing Evaluator code,
benchmark launch logic, plugin skills, or CI.
