...
# tracer output may live on another node / not be staged into this runner.
...
# Empty job_dir on a fresh checkout is normal — tracer output may not be
...
if [[ ! -d "$JOB_DIR" ]]; then
  ...
  echo "      (tracer output may live elsewhere; conversion would fail without it)"
  exit 77
fi
