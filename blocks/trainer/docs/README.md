...
User-facing documentation for the [trainer](../CLAUDE.md) block, built as a
...
Pages project `legoflow-trainer-docs` and prints the URL Cloudflare assigned.
...
training-progress webui is a separate React + `server.py` app under
...
Builds and deploys `out/` to the `legoflow-trainer-docs` Cloudflare Pages project.
It activates Node 22 via nvm,
...
│   ├── getting-started.mdx
│   ├── core-concepts.mdx
│   ├── data-pipeline/     #   conversion pipeline, scaffolds, scoring
│   ├── training/          #   run the pipeline, configuration, results & artifacts
│   ├── dashboard.mdx
...
2. Add its slug to the folder's `meta.json` `pages` array to place it in the
   sidebar order.
3. Link to other pages by their route, e.g. `/docs/training/configuration`.
