...
if [ "$TUNNEL" = "true" ]; then
  # PATH first, then the usual local install locations.
  CLOUDFLARED_BIN="${CLOUDFLARED_BIN:-$(command -v cloudflared 2>/dev/null || true)}"
  if [ -z "$CLOUDFLARED_BIN" ]; then
    for candidate in "$HOME/.local/bin/cloudflared" /usr/local/bin/cloudflared /opt/cloudflared/bin/cloudflared; do
      [ -x "$candidate" ] && { CLOUDFLARED_BIN="$candidate"; break; }
    done
  fi
  if [ -z "$CLOUDFLARED_BIN" ]; then
