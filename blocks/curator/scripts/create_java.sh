...
STATE_DIR="${PROJECT_ROOT}/artifacts/state/legoflow-curator-java"
mkdir -p "$STATE_DIR"
cd "$STATE_DIR"
...
legoflow-curator create \
  ...
  --min-source-files 2 \
  --max-source-files 10 \
  2>&1 | tee "${PROJECT_ROOT}/artifacts/logs/legoflow-curator-create/cc_java_$(date -u +%Y%m%dT%H%M%SZ).txt"
