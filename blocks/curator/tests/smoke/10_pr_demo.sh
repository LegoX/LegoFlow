...
STATE="$BLOCK_DIR/artifacts/swe_tasks/.legoflow-curator-smoke-py"
LOG="$BLOCK_DIR/artifacts/swe_tasks/.legoflow-curator-smoke-py.log"

mkdir -p "$OUTPUT" "$STATE"
: > "$LOG"

# If config.yaml selects cc_provider_mode: openai_proxy, legoflow-curator's Claude Code
...
echo "INFO: running legoflow-curator create on 10 PRs (output=$OUTPUT)"
echo "INFO: log -> $LOG"
...
timeout --foreground 3600 \
  "$VENV_BIN/legoflow-curator" create \
...
if [[ "$rc" == 124 ]]; then
  ...
  echo "FAIL: legoflow-curator create finished (rc=$rc) but verifiable_tasks.txt is empty"
fi
