...
> `meta_info.dependencies` is a mapping with exactly two keys, `from` and `to` — a block declares both directions it participates in, from its own file. A block with neither declares an explicit `dependencies: {from: {}, to: {}}`. The root's `subblocks` entries carry roles only — never wiring.
