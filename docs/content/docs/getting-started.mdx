...
`/root:check` and `/root:run` apply the same lifecycle across the whole tree,
with a free-form argument that selects the target block. An explicit
`/root:run <name>` delegates to `/<name>:run` and waits instead of bypassing the
block-owned skill:

```text
/root:check                         # check root + every subblock
/root:check curator                  # one block at a time
/root:run tracer                     # delegates to /tracer:run and waits
