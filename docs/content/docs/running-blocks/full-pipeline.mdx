...
user-owned and every change requires confirmation. Replace `${INSTRUCTION}`
with an experiment brief like the one below. It asks the agent to inspect the
available resources instead of assuming a fixed model, machine, or run size.
...
The same instruction is available as a standalone Markdown file:
[LegoFlow-new-user-example-prompt.md](/prompts/LegoFlow-new-user-example-prompt.md).
...
| --- | --- | --- |
| Curator | Verified SWE tasks | Only tasks that pass NOP and Oracle validation reach the manifest. |
