...
A **block** is the unit of work in LegoFlow. Every stage of the pipeline — data curation, trajectory generation, training, and evaluator measurement — is a block, with the same shape and the same three skills (`:setup`, `:check`, `:run`). This page is the long-form design: the vocabulary, the schema, the dependency rules, and the archive format.
...
The repo is a **tree of blocks**. The repo root is the **root block**; every directory under `blocks/` is a child block. A parent block lists its children under `meta_info.blocks` (with a role one-liner each); each child declares its own upstream AND downstream hand-offs in its `meta_info.dependencies` (`from`/`to`). Children can have children — the abstraction nests.
