...
- **`/root:run`** — delegate an explicit subblock target to its `/<name>:run`
  skill and wait. With no target, a parent dispatches to child run skills; only
  a leaf invoked from its own directory may execute `scripts/start.sh`.
