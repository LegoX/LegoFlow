...
#
#   bash tests/smoke/verify.sh <swegen|trajgen|sft|eval>
#
...
# quality):
#   swegen  : >=1 verified task in the smoke subdir's verifiable_tasks.txt
#   trajgen : >=1 SCORED trial (pipeline health, like the subblock smoke);
#             reward==1 (resolved) + converted lf.json are reported, NOT gated
#   sft     : train_results.json (finite loss) AND a persisted checkpoint
#             (config.json + weights) in the run dir
#   eval    : >=1 clean scored trial; reports resolved/total over the 100-subset
#
# Exit 0 = PASS, 77 = SKIP (prereq absent — stage never really ran), 1 = FAIL.

set -uo pipefail

STAGE="${1:?usage: verify.sh <swegen|trajgen|sft|eval>}"
...
case "$STAGE" in
  ...
  swegen)
    ...
    if [[ ! -d "$B/$BASE/$SUB" ]]; then
      echo "SKIP: swegen output dir $BASE/$SUB absent — create step never ran"
      exit 77
    fi
    if [[ -s "$MANIFEST" ]]; then
      n=$(wc -l <"$MANIFEST")
      echo "PASS: swegen produced $n verified task(s):"
      sed 's/^/         /' "$MANIFEST"
      exit 0
    fi
    echo "FAIL: $MANIFEST empty/missing — no verified tasks to hand to trajgen"
    exit 1
    ;;

  # --------------------------------------------------------------- trajgen ----
  trajgen)
    B="$ROOT_DIR/subblock/trajgen"
    ...
    JOBS_DIR="$B/$JOBS"
    if [[ ! -d "$JOBS_DIR" ]]; then
      echo "SKIP: trajgen jobs dir $JOBS absent — harbor never ran"
      exit 77
    fi
    # reward determination mirrors the subblock trajgen smoke (verify.sh):
...
PY
    ...
    # reward==1 trajectories converted to LF for sft (reported, not gated).
    ...
    # PASS gate = PIPELINE HEALTH (>=1 SCORED), exactly like the subblock trajgen
    ...
    # trajectories exist to sft; sft falls back to the 512 fixture if there are 0.
    if [[ "$scored" -ge 1 ]]; then
      echo "PASS: trajgen pipeline healthy — $scored scored trial(s); RESOLVED(reward>0)=$resolved; reward==1 lf.json=$LF_COUNT"
      exit 0
    fi
  ...
  # ------------------------------------------------------------------- sft ----
  sft)
    B="$ROOT_DIR/subblock/sft"
    OUT="$(cfg "$B/config.yaml" runtime_info.input.training.output_dir)"; OUT="${OUT:-root_smoke_model}"
    RUN_DIR="$B/artifacts/model/$OUT"
    if [[ ! -d "$RUN_DIR" ]]; then
      echo "SKIP: sft run dir artifacts/model/$OUT absent — training never produced output (likely remote-only; run_pipeline fetches train_results.json)"
      exit 77
    fi
...
PY
    ...
    [[ "$VERDICT" == OK* ]] && { echo "PASS: sft ($VERDICT)"; exit 0; }
    echo "FAIL: $VERDICT"
    exit 1
    ;;
  ...
  # ------------------------------------------------------------------ eval ----
  eval)
    B="$ROOT_DIR/subblock/eval"
    JOBS="$(cfg "$B/config.yaml" runtime_info.input.harbor_job.jobs_dir)"; JOBS="${JOBS:-artifacts/jobs/root-smoke}"
    JOBS_DIR="$B/$JOBS"
    if [[ ! -d "$JOBS_DIR" ]]; then
      echo "SKIP: eval jobs dir $JOBS absent — eval never launched"
      exit 77
    fi
    SCAN="$(JOBS_DIR="$JOBS_DIR" python3 - <<'PY'
import json, os, glob
...
PY
    ...
    if [[ "$clean" -ge 1 ]]; then
      echo "PASS: eval scored the trained model on the verified subset ($SCAN)"
      exit 0
    fi
    echo "FAIL: 0 clean scored trials — every eval trial errored ($SCAN)"
    exit 1
    ;;

  *)
    echo "usage: verify.sh <swegen|trajgen|sft|eval>" >&2
    exit 2
    ;;
esac
