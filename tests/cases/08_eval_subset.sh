#!/usr/bin/env bash
# Root case 08: eval smoke targets the swebench-verified 100-task subset and is
# wired to serve the sft checkpoint via remote vLLM + LiteLLM.
...
# serving handoff (remote GPU pod -> vLLM -> LiteLLM -> eval) is declared.
...
cfg = yaml.safe_load(open(os.path.join(sys.argv[1], "tests/smoke/eval/config.yaml"), encoding="utf-8")) or {}
...
# Serving handoff: the trained checkpoint is served on the sft GPU pod via
# vLLM and wrapped by LiteLLM; eval points its llm_api at that wrapper.
...
if not serving.get("vllm"):
    errs.append("serving.vllm block missing (how to launch vLLM on the sft pod)")
if not serving.get("litellm"):
    errs.append("serving.litellm block missing (how to wrap vLLM for eval)")
host_ref = str(serving.get("host", ""))
# host is resolved from the sft block's resources at run time; the config should
# say so rather than hardcode a stale IP.
if host_ref and host_ref not in ("sft.resources", "from_sft_resources"):
    errs.append(f"serving.host={host_ref!r}; expected to reference the sft pod (from_sft_resources)")

if errs:
    for e in errs: print("FAIL:", e, file=sys.stderr)
    sys.exit(1)
print(f"PASS: eval smoke -> swebench-verified n_tasks={n}, served from sft checkpoint")
PY
