#!/usr/bin/env bash
# Root case 07: swegen smoke is sized for the 200-PR from-scratch collection.
...
cfg = yaml.safe_load(open(os.path.join(sys.argv[1], "tests/smoke/swegen/config.yaml"), encoding="utf-8")) or {}
...
# max_pr = stop after N verified tasks. Root smoke wants a handful so trajgen
...
if errs:
    ...
    sys.exit(1)
print(f"PASS: swegen smoke sized for from-scratch collection (target_prs={target}, max_pr={max_pr})")
PY
