...
This page walks through preparing the sft block and running a training job end to end. sft wraps a pinned [SWE-Lego LLaMA-Factory](https://github.com/SWE-Lego/LLaMA-Factory) checkout and the `swe_data_process` converter package, so most setup is about getting those into a single `uv` environment.
