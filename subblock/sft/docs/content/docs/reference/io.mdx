...
When `source.type` is `harbor_job` (the default), `source.job_dir` is the handoff from trajgen, wired via `meta_info.dependencies.trajectories_dir` (`trajgen.output.raw_trajectories_dir`). For `hf_lf` / `local_lf` the input is a ready-made LF/ShareGPT dataset and conversion is skipped — see [Input sources](/docs/data-pipeline#input-sources).
