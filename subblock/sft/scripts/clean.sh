...
# Keeps: env/, envs/, index.yaml, archives/, data/, model/ under artifacts/.
# data/ and model/ are primary outputs — pass --outputs to remove them too.
set -euo pipefail
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Removes intermediates under $ARTIFACTS_DIR (logs/, wandb/, training_config/, etc.)."
            echo "Pass --outputs to also remove data/ and model/."
            exit 0
            ;;
...
for entry in "$ARTIFACTS_DIR"/*; do
    ...
    case "$name" in
        ...
        data|model) [[ "$REMOVE_OUTPUTS" == "0" ]] && continue ;;
    esac
    if [[ "$DRY_RUN" == "1" ]]; then
