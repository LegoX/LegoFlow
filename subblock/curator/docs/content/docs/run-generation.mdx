...
These are separate stages: `/curator:run` never starts the collector. Wait for
the background collector to exit before running generation. Production create
scripts read fixed paths under `artifacts/collected_prs/`; changing
`pr_collection.output_dir` requires a corresponding manual `swegen create`
input path. The sections below document the raw commands those skills run.
...
`pr_collection.enabled` is currently informational; the wrapper runs whenever
you invoke it.
...
collection tokens are combined from `repos/swegen/gh_token.txt` (or
`COLLECT_GITHUB_TOKEN_FILE`) and `GITHUB_TOKENS` / `GITHUB_TOKEN`; they never
come from `config.yaml`. Per-language threshold overrides live in the
collector's `LANGUAGE_OVERRIDES` and take precedence over the global `filters`.
...
Use the smaller command above for collector validation. It does not feed
`/curator:run` smoke mode, which uses the sample PR file bundled in
`repos/swegen`.
...
Setting `N_CONCURRENT=1` in front of the script does not override the configured
value because `read_params.py` assigns it afterward. Script logs are written to:
...
The script always launches all eight language create scripts with `nohup`,
regardless of their `enabled` values, and returns without waiting. Use the
individual `scripts/create_<lang>.sh` scripts to limit the language set. Monitor
the detached workers through their logs, batch state, and the dashboard.

The mode-specific wrappers delegate to this same background launcher. Their
`artifacts/index.yaml` archive records launcher exit, not worker completion.
`start_with_openai_api.sh` also stops its LiteLLM proxy as soon as the launcher
returns, so it does not supervise that proxy for the detached workers' full
lifetime.
...
Each language script assigns `N_CONCURRENT` from `config.yaml` after loading the
environment, so a shell prefix such as `N_CONCURRENT=8` is overwritten. Update
the language's `params.n_concurrent` value in `config.yaml` instead.
