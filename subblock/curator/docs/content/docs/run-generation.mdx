...
`config.yaml -> runtime_info.input.pr_collection`; `/curator:create-tasks`
reads per-language `timeout` / `cc_timeout` / `n_concurrent` from
`runtime_info.input.languages`. Edit `config.yaml`, then re-run the command.
These are separate stages: `/curator:create-tasks` never starts the collector.
Wait for the background collector to exit before running generation. Production
create scripts read fixed paths under `artifacts/collected_prs/`; changing
...
`/curator:create-tasks` smoke mode, which uses the sample PR file bundled in
`repos/swegen`.
