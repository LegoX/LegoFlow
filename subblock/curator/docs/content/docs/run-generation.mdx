...
pr_collection:
  ...
  token_limit: 32                  # parallel GitHub API workers
  filters:
    min_stars: 30
    min_merged_prs: 5
...
languages:
  ...
  java: { enabled: true,  params: { timeout: 4000, cc_timeout: 3000, n_concurrent: 12 } }
  c:    { enabled: true,  params: { timeout: 3200, cc_timeout: 2400, n_concurrent: 16 } }
  cpp:  { enabled: true,  params: { timeout: 3600, cc_timeout: 3000, n_concurrent: 12 } }
  rust: { enabled: true,  params: { timeout: 3600, cc_timeout: 3000, n_concurrent: 12 } }
```
...
Scripts read parameters from `config.yaml` automatically.
