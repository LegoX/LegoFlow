...
# Keeps: env/, envs/, index.yaml, archives/, swe_tasks/ under artifacts/.
# swe_tasks/ is a primary output — pass --outputs to remove it too.
set -euo pipefail
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "Removes intermediates under $ARTIFACTS_DIR (logs/, collected_prs/, etc.)."
            echo "Pass --outputs to also remove swe_tasks/."
            exit 0
            ;;
...
for entry in "$ARTIFACTS_DIR"/*; do
    ...
    case "$name" in
        ...
        swe_tasks) [[ "$REMOVE_OUTPUTS" == "0" ]] && continue ;;
    esac
    if [[ "$DRY_RUN" == "1" ]]; then
