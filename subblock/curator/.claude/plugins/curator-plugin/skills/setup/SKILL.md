...
name: setup
description: >
  Bootstrap the curator block from a fresh clone: initialise the swegen
...
Prepare `subblock/curator/` so `/curator:check` can pass. The operation is
...
If the current directory is not `subblock/curator/`, stop and tell the user
...
Then return to `subblock/curator/`. If `config.yaml` declares a non-null
