...
You (the AI agent) monitor and tune the Curator pipeline. Configuration and per-language status live in `config.yaml` under `runtime_info.input.languages.<lang>` and `runtime_info.input.global`.
