#!/usr/bin/env bash
# Validate evaluator config and local Harbor repo state without running rollouts.
# Mirrors tracer/scripts/dryrun.sh, but task_source is registry-based (no
...
echo "=== evaluator dryrun: $BLOCK_DIR ==="
echo ""
...
case "$PROVIDER" in
  harbor_registry)
    ...
    ;;
  local|huggingface)
    warn "task_source.provider=$PROVIDER is not the evaluator-block default; expected harbor_registry"
    ;;
  *)
    fail "unsupported task_source.provider for evaluator block: $PROVIDER (expected harbor_registry)"
    ;;
esac
