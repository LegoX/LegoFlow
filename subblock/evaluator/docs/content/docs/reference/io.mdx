...
evaluator is **standalone** — `meta_info.dependencies` is empty. The benchmark comes from Harbor's registry, not an upstream block, so the only values you normally fill are `llm_api` (or `local_model_serving` for a local checkpoint) and `task_source`.
