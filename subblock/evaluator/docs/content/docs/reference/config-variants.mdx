...
The knobs worth tuning between runs all live under `runtime_info.input` in `config.yaml`. For evaluator, the **primary lever is benchmark selection** (`task_source.dataset_name`); the rest mirror tracer:

| Parameter | Meaning |
| --- | --- |
| `task_source.no_hack` | `true` = run hardened `swebench-verified-nohack` (agent egress → LiteLLM only) |
