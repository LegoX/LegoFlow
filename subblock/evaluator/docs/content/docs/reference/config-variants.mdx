...
The knobs worth tuning between runs all live under `runtime_info.input` in `config.yaml`. For eval, the **primary lever is benchmark selection** (`task_source.dataset_name`); the rest mirror tracer:
