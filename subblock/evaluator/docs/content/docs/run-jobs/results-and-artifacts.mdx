...
`jobs/`, `datasets/`, and `runtime/`. Pass `-n`/`--dry-run` to preview the
deletions. It does not touch `repos/`; remove sensitive job/archive data
explicitly before packaging a worktree.
