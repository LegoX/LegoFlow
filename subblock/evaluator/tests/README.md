...
401/403 or 400/404→WARN) lives in `/evaluator:check` and is exercised for real by the
smoke. See memory `project-eval-llm-endpoint`.

### How evaluator differs from tracer's suite

evaluator is the **registry-driven, terminal** block, so the suite drops three
tracer cases and adds two:
