...
# evaluator bind-mounts this host dir read-only into every task container at
...
if [[ ! -d "$ABS" ]]; then
  echo "FAIL: agent.runtime_host_path does not exist: $RUNTIME_HOST_PATH_RAW (re-extract via /evaluator:setup)"; exit 1
fi
if [[ -z "$(ls -A "$ABS" 2>/dev/null)" ]]; then
  echo "FAIL: agent.runtime_host_path is empty: $RUNTIME_HOST_PATH_RAW (re-extract via /evaluator:setup)"; exit 1
fi
if [[ -z "$MARKER" ]]; then
