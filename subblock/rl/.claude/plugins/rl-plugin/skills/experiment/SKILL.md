...
description: >
  ...
  an empty notes file. Does NOT launch training (use /rl:run for that), and
  does NOT create blocks (that is /root:create, root plugin only).
...
This is an rl-specific extra beyond the uniform block skill set
(`setup`/`check`/`dashboard`/`run`). It scaffolds *experiment slots*, not
blocks — new blocks are created only via the root plugin's `/root:create`.
...
1. `./config.yaml` exists, parses, and has `meta_info.name == 'rl'`. If not,
   abort: "/rl:experiment must be run from inside the rl block (`subblock/rl/`)."
...
actual:
  # fill in after the run completes (the matching artifacts/index.yaml
  # entry is written automatically by archive_run.sh) — leave null here
  started_at: null
  completed_at: null
  exit_status: null
...
`/rl:run`. (`/rl:experiment` does **not** mutate the live config.)
...
- Do not append to `./artifacts/index.yaml` — `archive_run.sh` (via `start.sh`'s EXIT trap) writes it when a run actually happens.
- Do not launch training, set up venvs, or call any upstream script.
- Do not modify anything under `repos/`.
