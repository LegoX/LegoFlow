...
# This mirrors eval's "clean scored trial" semantics.
#
# Resolved (reward>0) is reported but does not gate: at trajgen's smoke scale
...
if [[ "$scored" -ge 1 ]]; then
  echo "PASS: trajgen pipeline produced >=1 scored trial ($SCAN)"
  exit 0
fi
