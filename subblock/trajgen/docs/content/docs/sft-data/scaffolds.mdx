...
The **scaffold** is the agent harness that produced a trajectory. Because each scaffold logs in its own shape, the converter that reshapes a trajectory into IM format is scaffold-specific. trajgen ships converters for the four scaffolds below via the [`swe_data_process`](https://github.com/SWE-Lego/swe_data_process) package.
