...
The SFT block already expects `source_block: trajgen` for raw trajectories.
The handoff fields are:

- `config.yaml -> harbor_job.jobs_dir`
- derived latest job dir: `{harbor_job.jobs_dir}/latest`
- derived producer/output format: `trajgen` / `litellm_logger_v0.1`
- derived trajectory file pattern: `{job_dir}/<task-id>/agent/litellm-trajectory.jsonl`

Copy those values into `subblock/sft/inputs.yaml` when preparing an SFT run.
