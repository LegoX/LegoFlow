...
name: check
description: >
  Preflight the trajgen block. Validates config.yaml schema; verifies
  ...
  trajgen", "preflight trajgen", "is trajgen ready", "diagnose trajgen",
  "validate trajgen config".
---
...
1. Execute `bash scripts/dryrun.sh` from the block root. The script is
   the single source of truth for what "trajgen is ready" means — it
