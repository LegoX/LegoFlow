...
HOME_DIR="${TERMINALGEN_HOME:-$HOME}"
DASHBOARD_SCRIPT="$SCRIPT_DIR/progress_monitor_all.py"
RUN_DIR="$SCRIPT_DIR"
PUBLIC_DIR="${PUBLIC_DIR:-$RUN_DIR/site}"
STATE_FILE="${STATE_FILE:-$RUN_DIR/memory/.progress_monitor_all_state.jsonl}"
CACHE_FILE="${CACHE_FILE:-$RUN_DIR/memory/.progress_monitor_all_cache.json}"
ENV_FILE="${ENV_FILE:-$HOME_DIR/.config/terminalgen_progress_cloudflare.env}"

PROJECT_NAME="${PROJECT_NAME:-terminalgen-databoard}"
BRANCH_NAME="${BRANCH_NAME:-terminalgen}"
...
LOCK_FILE="${LOCK_FILE:-/tmp/terminalgen_dashboard_sync.lock}"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
...
while true; do
  ...
  if [[ "$PROJECT_READY" -eq 0 ]]; then
    ...
    npx --yes wrangler pages project create "$PROJECT_NAME" \
      --production-branch "$BRANCH_NAME" >/tmp/terminalgen_pages_project_create.log 2>&1 || true
    PROJECT_READY=1
  fi
  ...
  npx --yes wrangler pages deploy "$PUBLIC_DIR" \
    ...
    --branch "$BRANCH_NAME" \
    --commit-dirty=true \
    --commit-message "Update terminalgen progress dashboard $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
