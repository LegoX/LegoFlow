#!/usr/bin/env bash
# Stop the terminalgen pipeline cleanly.
#
# Kills, in order:
#   1. the create_all_bg.sh launcher (if still alive),
#   2. each bash scripts/create_domain.sh worker,
#   3. any terminal-lego generator/validator python process spawned by those workers.
#
...
#   --with-docker    additionally stop docker containers whose name starts with
#                    `tl-val-` (the validator's container prefix; off by default)
...
# Argv patterns that uniquely identify terminalgen worker processes, anchored on
# this block dir so we never match a homonym process from another block/checkout.
PATTERNS=(
    "bash[[:space:]]+${BLOCK_DIR}/scripts/create_all_bg\.sh"
    "bash[[:space:]]+(\./)?scripts/create_domain\.sh"
    "${BLOCK_DIR}/repos/terminal-lego/(generator/task_generator|validator/validate_tasks)\.py"
)
...
if [ -z "$PIDS" ]; then
    echo "stop.sh: no terminalgen workers running."
else
...
if [ "$WITH_DOCKER" -eq 1 ]; then
    if ! command -v docker >/dev/null 2>&1; then
        ...
        CONTAINERS="$(docker ps --format '{{.ID}} {{.Names}}' 2>/dev/null \
            | awk 'tolower($2) ~ /^tl-val-/ {print $1}')"
        if [ -z "$CONTAINERS" ]; then
            echo "stop.sh: no tl-val-* containers running."
        else
