#!/usr/bin/env bash
# CI test runner for the terminalgen block.
#
# Usage:
#   bash tests/run.sh                 # cases/ only (cheap, no LLM tokens)
#   bash tests/run.sh --with-smoke    # cases/ then smoke/ (real LLM/Docker)
#
...
echo "================================================================="
echo "  terminalgen tests: PASS=$pass  SKIP=$skip  FAIL=$fail"
echo "================================================================="
if [[ "$fail" -gt 0 ]]; then
