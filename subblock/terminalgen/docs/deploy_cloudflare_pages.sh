#!/usr/bin/env bash
# Build the Terminal-gen docs (fumadocs/Next.js static export) and deploy to a
# dedicated Cloudflare Pages project. Independent of the data dashboard project
# (terminalgen-databoard); this one defaults to terminalgen-docs.
#
# Required credentials:
#   CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID
# read from $ENV_FILE (default ~/.config/terminalgen_docs_cloudflare.env), or from
...
PROJECT_NAME="${PROJECT_NAME:-terminalgen-docs}"
BRANCH_NAME="${BRANCH_NAME:-terminalgen}"
OUT_DIR="${OUT_DIR:-out}"
WRANGLER_PKG="${WRANGLER_PKG:-wrangler@latest}"
ENV_FILE="${ENV_FILE:-$HOME/.config/terminalgen_docs_cloudflare.env}"
