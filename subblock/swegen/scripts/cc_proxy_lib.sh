#!/usr/bin/env bash
# Shared helper: start/stop a local LiteLLM proxy for swegen's Claude Code (CC)
...
cc_proxy_start() {
  ...
  # proxy server import and skip the ones that can't serve. trajgen's
  ...
  if [ -z "$bin" ]; then
    local cand
    for cand in \
      "$block_dir/../trajgen/artifacts/env/litellm-venv/bin/litellm" \
  ...
  if [ -z "$bin" ] || [ ! -x "$bin" ]; then
    echo "ERROR: no proxy-capable litellm found (litellm[proxy] missing). Point" \
         "LITELLM_BIN at trajgen's litellm-venv or run: pip install 'litellm[proxy]'." >&2
    return 1
  fi
