...
description: >
  ...
  sample. This is the PR-collection stage that precedes `/swegen:run`.
...
stage; `/swegen:run` (task generation) consumes its output. Run only from
`subblock/swegen/`.
...
3. `repos/swegen/tools/collect_prs_wo_image.py` exists (submodule initialized;
   if missing, tell the user to run `/swegen:setup`).
...
| Scope | Use when | How |
| --- | --- | --- |
| `sample` | First run, "quick", "just a few", smoke before a full pass. | One language, small `repo_num` (e.g. 2) and `max_prs_per_repo` (e.g. 10) via env overrides — enough to feed a `/swegen:run` smoke. |
...
per line — the input to `swegen create` / `/swegen:run`.
...
- next step: `/swegen:run` to generate tasks from the collected PR ids
