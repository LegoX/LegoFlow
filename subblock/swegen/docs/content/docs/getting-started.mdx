...
`subblock/swegen/repos/swegen`.

## Recommended: operate via the Claude plugin

The block ships a Claude Code plugin (`swegen-plugin`) that drives the whole
lifecycle through slash commands. This is the recommended way to run swegen —
each command performs the preflight, confirmation, and bookkeeping steps for
you. Launch Claude from inside `subblock/swegen/` so the block-local plugin
loads, then:

```text
/swegen:setup        # init submodule, build venv, install swegen, dryrun
/swegen:check        # preflight: config, GitHub tokens, LLM endpoint, docker
/swegen:collect-prs  # collect GitHub PRs -> artifacts/collected_prs/{lang}_pr_ids.txt
/swegen:run          # generate + verify tasks -> verifiable_tasks.txt
/swegen:dashboard    # local preview or Cloudflare Pages sync
```
...
This prints the gitlink commit recorded for the submodule. `/swegen:setup`
