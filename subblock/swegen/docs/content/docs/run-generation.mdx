...
language-specific shell scripts in `subblock/swegen/scripts/`. Each script
...
From a Claude session started inside `subblock/swegen/`:

```text
/swegen:collect-prs   # collect PRs per config.yaml -> runtime_info.input.pr_collection
/swegen:run           # generate + verify tasks per runtime_info.input.languages
/swegen:dashboard     # monitor progress
```

`/swegen:collect-prs` reads its scope and filters from
`config.yaml -> runtime_info.input.pr_collection`; `/swegen:run` reads
