...
# subblock/swegen/config.yaml that doesn't have a `smoke:` section).
read_cfg() {
...
if [[ ! -d "$OUTPUT" ]]; then
  echo "SKIP: no $OUTPUT — claude /swegen:run smoke didn't get to the create step"
  exit 77
fi
