...
# Wall-clock budget: ~30 min when it works (tox-dev/tox:pr-3813 first).
#
...
# --max-pr 1 → bail as soon as one PR verifies. Walltime cap 30m via `timeout`.
set +e
...
if [[ "$rc" == 124 ]]; then
  echo "FAIL: 30-minute wall-clock budget exceeded; no verified task produced"
else
