...
Live board: **[swe-tracer-databoard-eir.pages.dev](https://swe-tracer-databoard-eir.pages.dev/)**
...
- **Operations** — latest archived-run status from `artifacts/index.yaml`, searchable Harbor/SFT artifact tables, copy-path actions, and per-job eval breakdowns.
...
Configuration can come from exported environment variables, the root
`config.yaml` → `runtime_info.input.cloudflare`, or the legacy
`~/.config/trajgen_progress_cloudflare.env` file (override its path with
`ENV_FILE`). Environment/file values take precedence over the shared root
config. The required values are:

| Variable | How to fill it | Default |
| --- | --- | --- |
| `CLOUDFLARE_API_TOKEN` | API token with account-level `Cloudflare Pages: Edit`; keep it out of tracked config | required |
| `CLOUDFLARE_ACCOUNT_ID` | 32-char Account ID from the dashboard; may live in root config | required |
...
The same loop optionally re-runs
`scripts/convert_trajectories.sh --skip-unchanged` at most every
`CONVERT_EVERY_SECONDS`. It derives a scaffold override from the target job
name when possible, so historical jobs do not silently use an unrelated
current `agent.name`.
