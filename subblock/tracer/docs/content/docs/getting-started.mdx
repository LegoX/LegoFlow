...
This page walks through preparing the tracer block and running a job end to end. tracer wraps a pinned [Harbor](https://github.com/SWE-Lego/harbor) checkout, so most steps are about getting that runtime and your task set in place.
...
- **Verified tasks from curator** — a task source containing a `verifiable_tasks.txt` manifest (see [Core Concepts](/docs/core-concepts#verified-task-and-manifest)).

<Callout title="Where to run" type="info">
tracer runs on the node declared in `config.yaml` (`meta_info.resources.ip`). Run its scripts inside a `tmux` session on that host so a job survives shell disconnects.
</Callout>

## 1. Update the managed repos

tracer depends on two local-only repositories, `harbor` and `swe_data_process`, pinned to specific commits in `config.yaml`. Clone or update them to the pinned refs:
...
/tracer:setup           # update repos, build envs, run dryrun
/root:run tracer       # delegate to /tracer:run and wait
# or invoke the same block-owned run skill directly:
/tracer:run             # tracer-specific run procedure + post-run bookkeeping
