...
tracer only ever rolls out tasks that curator has marked verified, and never rolls the same task out twice. Two mechanisms enforce this: the **manifest filter** during task preparation, and the **processed-tasks ledger** plus exclude list during a run.
...
curator tasks + verifiable_tasks.txt
   └─ manifest filter ─▶ artifacts/tasks/<dataset>
                           └─▶ Harbor job ◀── --exclude-task-name ── HARBOR_EXCLUDE_TASKS
                                  │                                          ▲
                                  └─ update entries ─▶ processed_tasks.yaml
                                                          (done / failed / skipped)
```
