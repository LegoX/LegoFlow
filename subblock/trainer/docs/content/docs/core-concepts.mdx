...
A **checkpoint** is a saved model state under `artifacts/model/<run>/checkpoint-<step>/`, written every `save_steps`. The latest checkpoint path is recorded in `config.yaml` → `runtime_info.output.checkpoint_path` and is the block's primary handoff to downstream consumers (`rl`, `evaluator`).
