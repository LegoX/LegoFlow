...
trainer ships a real-time, read-only web dashboard for monitoring LLaMA-Factory (HuggingFace Trainer) training runs. It is a React frontend + a Python `server.py` backend under [`dashboard/`](https://github.com/SWE-Lego/LegoFactory/tree/main/subblock/trainer/dashboard), parses the standard training output directly, and can compare multiple runs or connect to wandb.
...
See [`dashboard/README.md`](https://github.com/SWE-Lego/LegoFactory/tree/main/subblock/trainer/dashboard) for the full CLI options and environment variables.
