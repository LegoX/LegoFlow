...
When `source.type` is `harbor_job` (the default), `source.job_dir` is the handoff from tracer, wired via `meta_info.dependencies.trajectories_dir` (`tracer.output.raw_trajectories_dir`). For `hf_lf`, setting `hf_file_name` downloads and registers only that file; leaving it empty loads the Hub dataset config/split at train time. `local_lf` registers `lf_path` directly. See [Input sources](/docs/data-pipeline#input-sources).
