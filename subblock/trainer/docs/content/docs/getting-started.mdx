...
This page walks through preparing the trainer block and running a training job end to end. trainer wraps a pinned [LLaMA-Factory](https://github.com/hiyouga/LLaMA-Factory) checkout and the `swe_data_process` converter package, so most setup is about getting those into a single `uv` environment.
...
- **Raw trajectories from tracer** — a Harbor `job_dir` of agent rollouts (see [Core Concepts](/docs/core-concepts#trajectory)).
