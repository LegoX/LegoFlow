#!/usr/bin/env bash
# Build the eval docs (fumadocs/Next.js static export) and deploy to a
# dedicated Cloudflare Pages project. Independent of any dashboard project;
# this one defaults to swe-eval-docs.
#
# Reuses the same Cloudflare credentials as the trajgen docs/dashboard sync:
...
PROJECT_NAME="${PROJECT_NAME:-swe-eval-docs}"
BRANCH_NAME="${BRANCH_NAME:-eval}"
OUT_DIR="${OUT_DIR:-out}"
...
# The shared env file (e.g. .env.cf) may set PROJECT_NAME/BRANCH_NAME for a
...
# otherwise the docs would deploy over another project.
