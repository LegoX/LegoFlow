...
This page walks through preparing the eval block and running a benchmark end to end. eval wraps a pinned [Harbor](https://github.com/SWE-Lego/harbor) checkout, so most steps are about getting that runtime in place — there is no task staging step, because the benchmark is resolved from Harbor's registry.
