...
name: dashboard
description: >
  Per-job status surface for eval: read `artifacts/jobs/<job>/` to count
  ...
  Read-only. Triggers on phrases like "eval dashboard", "show eval
  progress", "swebench accuracy", "how many tasks resolved", "what eval
  jobs are running".
---
...
3. **Optional webui** (later): reuse Harbor's job-view UI if available,
   or build a lightweight aggregator scoped to eval.
