...
def report_openai(args, ok, result, fix):
    """Print the OpenAI-path verdict. -> exit code, or None to keep going."""
    if ok:
        preview = result["text"].replace("\n", " ")[:60]
        print(f"PASS: {args.label} returned text ({result['base']}, "
              f"model={result['model']}): {preview!r}")
        return None
    if transient(result) and not fix:
...
def main() -> int:
    ...
    if args.anthropic_only:
        unfilled = is_placeholder(args.anthropic_base_url) or is_placeholder(
            args.anthropic_model or args.model)
    else:
        unfilled = is_placeholder(args.base_url) or is_placeholder(args.model)
    if unfilled:
        print(f"SKIP: {args.label} not filled in yet (base url/model still a placeholder)")
        return 77
    ...
    if not args.anthropic_only:
        ...
        rc = report_openai(args, ok, result, fix)
        if rc is not None:
            return rc
    ...
    if args.anthropic_base_url and not is_placeholder(args.anthropic_base_url):
        ...
        elif transient(result_a) and not fix_a:
            print(f"SKIP: Anthropic path looks transiently unavailable, not misconfigured "
                  f"— {describe(result_a)} at {result_a['base']}")
            return 77
        else:
